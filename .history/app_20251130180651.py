...
csv_bytes.seek(0)
# Compatibility: use `download_button` when available; otherwise render a base64 download link
try:
    # Some Streamlit versions provide download_button; try it and fall back on any error
...
if page == "Application":
    ...
    # pick data source: uploaded file or sample
    ...
    # Note: numeric inputs live in the sidebar for a cleaner experience
    st.write('Use the sidebar to change `Lead time`, `Service level` and cost parameters. Results update automatically.')
# Subheader
...
#Subheader
    ...
    cols_metrics[0].metric('Safety Stock', f"{int(round(Safety_Stock))}")
    cols_metrics[1].metric('Reorder Point', f"{int(round(Reorder_Point))}")
    cols_metrics[2].metric('Economic Order Qty', f"{int(round(EOQ))}")
