...
#Subheader
    ...
    try:
        ...
        preds = model.predict(testdata=testdata, model='best', simple=True)
        # preds may be a list/array or a dataframe/series - try to get numeric values
        if hasattr(preds, '__iter__') and not isinstance(preds, (str, bytes)):
            forecast_demand = float(np.average(list(preds)))
        else:
            forecast_demand = float(preds)
    except Exception:
